...
# from .colmap_read_model import * as read_model
...
def extract_transformation_matrices(poses):
    num_images = poses.shape[2]
    transformation_matrices = []

    for i in range(num_images):
        pose = poses[:, :, i]
        R = pose[:, :3]  # Rotation matrix (3x3)
        t = pose[:, 3:4]  # Translation vector (3x1)
        hwf = pose[:, 4]  # Height, width, and focal length (3)
    ...
    return transformation_matrices, hwf           
...
def gen_poses(basedir, match_type, factors=None):
    ...
    """ colmap 추정 행렬을 4x4 변환 행렬로 생성"""
    # Extract rotation matrices and translation vectors
    transformation_matrices, hwf = extract_transformation_matrices(poses)
