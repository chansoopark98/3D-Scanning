...
if __name__ == '__main__':
    ...
    pcds = []
    dir_name = '2023_03_22_18_45_47'
    camera_poses = gen_poses('./360degree_pointclouds/{0}/'.format(dir_name), 'exhaustive_matcher')
    
    len_poses = len(camera_poses)
    print('length of camera pose lists {0}'.format(len_poses))
    for i in range(len_poses):
        ...
        inv_pose = np.linalg.inv(camera_poses[i])
        relative_transformation_matrix = np.dot(inv_pose, camera_poses[i])
        pcd.transform(relative_transformation_matrix)
